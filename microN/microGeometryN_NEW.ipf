...
Function MakeMicroPanel(tab)							// makes the main microPanel
	...
	if (WinType("micropanel")==7)
		...
		NewPanel /W=(672,60,966,696+3*18)/K=1/N=microPanel as "LaueGo"
...
ThreadSafe Static Function/C tthRange(d,[t1,t2,t3])	// tth is the usual 2-theta
	...
	Variable px0,py0
	XYZ2pixel(d,ki,px0,py0, t1=t1,t2=t2,t3=t3)	// check to see if detector includes direct beam
...
ThreadSafe Static Function initialize_microGeometryStruct(g)
	STRUCT microGeometry &g					// set all values in a microGeometry structure to "empty"
	g.Ndetectors = 0
	initialize_sampleStruct(g.s)
	initialize_wireStruct(g.wire)
	Variable i
	for (i=0;i<MAX_Ndetectors;i+=1)
		initialize_detectorStruct(g.d[i])
	endfor
End
//
ThreadSafe Static Function initialize_detectorStruct(d)
	...
	d.detectorID = ""
	d.distortionMapFile = ""
	d.rho00 = 0		;	d.rho01 = 0		;	d.rho02 = 0
	d.rho10 = 0		;	d.rho11 = 0		;	d.rho12 = 0
	d.rho20 = 0		;	d.rho21 = 0		;	d.rho22 = 0
End
//
ThreadSafe Static Function initialize_sampleStruct(s)
	STRUCT sampleGeometry &s					// set all values in a sample structure to "empty"
	s.O[0] = NaN	;	s.O[1] = NaN	;	s.O[2] = NaN
	s.R[0] = NaN	;	s.R[1] = NaN	;	s.R[2] = NaN
	s.Rmag = NaN
	s.R00 = NaN		;	s.R01 = NaN		;	s.R02 = NaN
	s.R10 = NaN		;	s.R11 = NaN		;	s.R12 = NaN
	s.R20 = NaN		;	s.R21 = NaN		;	s.R22 = NaN
End
//
ThreadSafe Static Function initialize_wireStruct(w)
	STRUCT wireGeometry &w					// set all values in a wire structure to "empty"

	w.origin[0] = NaN		;	w.origin[1] = NaN		;	w.origin[2] = NaN
	w.dia = NaN
	w.knife = 0
	w.F = NaN
	w.axis[0] = NaN		;	w.axis[1] = NaN		;	w.axis[2] = NaN
	w.axisR[0] = NaN		;	w.axisR[1] = NaN		;	w.axisR[2] = NaN
	w.R[0] = NaN			;	w.R[1] = NaN			;	w.R[2] = NaN
	w.Rmag = NaN

	w.R00 = NaN		;	w.R01 = NaN		;	w.R02 = NaN
	w.R10 = NaN		;	w.R11 = NaN		;	w.R12 = NaN
	w.R20 = NaN		;	w.R21 = NaN		;	w.R22 = NaN
End
...
ThreadSafe Function/C q2pixel(d,qvec,[depth,t1,t2,t3])	// returns pixel position as a complex number cmplx(px,py)
	STRUCT detectorGeometry &d
	...
	XYZ2pixel(d,kout,px,py,depth=depth, t1=t1,t2=t2,t3=t3)
...
ThreadSafe Function/WAVE pixel2qVEC(d,pxpy,[depth,DeltaPixelCorrect,t1,t2,t3])	// returns the qhat in beam line coords
	STRUCT detectorGeometry &d
	...
	Variable depth							// OPTIONAL, sample depth measured along the beam (optional, 0 is default)
	...
	if (!ParamIsDefault(DeltaPixelCorrect) && DimSize(DeltaPixelCorrect,1)==2)
		...
		Wave kf = pixel2XYZVEC(d,pxpy, t1=t1,t2=t2,t3=t3)	// kf is in direction of pixel in beam line coords
	endif
...
ThreadSafe Function/WAVE pixel2XYZVEC(d,pxpy,[DeltaPixelCorrect,t1,t2,t3])	// convert pixel position to the beam line coordinate system
	STRUCT detectorGeometry, &d
	Wave pxpy							// list of pixels to use (must be in raw un-binned pixels), perhaps an ROI, first dimension is number of pixels, second is x,y
	Wave DeltaPixelCorrect			// OPTIONAL, pixel corrections dimensions are [N][2]
	Variable t1,t2,t3					// OPTIONAL, position of detector translators
	...
	elseif (N==1)
		...
		pixel2XYZ(d,pxpy[0][0],pxpy[0][1],xyz, t1=t1,t2=t2,t3=t3)	// convert pixel position to the beam line coordinate system
...
ThreadSafe Function pixel2q(d,px,py,qhat,[depth,t1,t2,t3])	// returns theta (rad)
	STRUCT detectorGeometry &d
...
ThreadSafe Function pixel2XYZ(d,px,py,xyz,[t1,t2,t3])	// convert pixel position to the beam line coordinate system
	STRUCT detectorGeometry, &d
...
ThreadSafe Function/C XYZ2pixel(d,xyz,px,py,[depth,t1,t2,t3])	// find pixel position (px,py) where vector xyz will intercept detector
	STRUCT detectorGeometry, &d
	...
	xyzp[0] = d.rho00*xx + d.rho10*yy + d.rho20*zz -d.P[0]	// so   xyz' = rho x xyz - P
	...
	dxyzP = -d.P[p] - xyzp[p]					// Ær', subtract origin from xyzp[3], this is direction of -kf

	if (depth!=0)									// a non-zero depth, so modify xyzp[3] to shift the origin by depth (shifts r')
		xyzp[0] += d.rho20*depth				// remember:   xyz' = ( rho x xyz ) - P
		xyzp[1] += d.rho21*depth				// shifted xyzp[3] by depth*ki, all in prime space
		xyzp[2] += d.rho22*depth				// This assumes that ki = {0,0,1}
	endif

	// find t,  where the line intercepts the detector
	Variable t = -xyzp[2]/dxyzP[2]			// for t==0, the point (x'y'z') already lies on the plane
	if (t>1)											// ray pointing backwards through other side of origin, so going away from detector
		...
		xx = xyzp[0] + t*dxyzP[0]
		yy = xyzp[1] + t*dxyzP[1]
		// zz = xyzp[2] + t*dxyzP[2]				// zz should be zero, that was the whole point! So this statement not needed.
		px = xx/d.sizeX*d.Nx + 0.5*(d.Nx-1)	// convert (xy0) in prime space to pixels
		py = yy/d.sizeY*d.Ny + 0.5*(d.Ny-1)
	endif
...
Static Function GeoFromXML(buf,g)
	...
	initialize_microGeometryStruct(g)	// set to "empty" or bad values
...
Function GeoReferenceOrientation(g[,simple])	// sets g to the reference orientation (sort of an ideal set of values)
	...
	//	g.d[0].m1[...] // was set to NaN by the initialize_microGeometryStruct(g) above
...
// NOTE:  This does not use the detector tranlations (m1,m2,m3) since it has no way to get the translator values {t1,t2,t3}
...
End
//
Static Function calcHofWire(win,whoChanged,[t1,t2,t3])
	String win
	...
	if ((i0+i1)==3)							// calc Hwire from depth & px
		pixel2XYZ(g.d[dnum],px,py,xyz, t1=t1,t2=t2,t3=t3)	// convert pixel position to the beam line coordinate system
	...
	elseif ((i0+i1)==6)						// calc px from depth & Hwire
		...
		Xw[1] = HF2Y(Hwire,g.wire.F)		// Y =  H*sin(angle) - F*cos(angle)
		Xw[2] = HF2Z(Hwire,g.wire.F)		// Z =  H*cos(angle) + F*sin(angle)
		px = WireDepth2Pixel(g,dnum,py,Xw,depth,edge, t1=t1,t2=t2,t3=t3)	// find px, given the wire and depth
...
ThreadSafe Static Function WireDepth2Pixel(g,dnum,py,Xw,depth,edge, [t1,t2,t3])	// find px, given the wire, depth, & py
	STRUCT microGeometry &g		// g.wire.F provides the F
...
Function/T FillGeometryParametersPanel(strStruct,hostWin,left,top)	// populate the controls in the Geo Panel
	...
	if (strlen(strStruct)>0)
		SetGeoStructFromString(g,strStruct)				// found structure information, load into geo
...
//	NewPanel/K=1/W=(left,top,left+221,top+603+3*18)/HOST=$hostWin
	...
	if (mUseShow>=1)
		...
		SetVariable m1x,limits={-1,1,0}, bodyWidth=50
		SetVariable m1y,pos={85,76+v},size={55,15},proc=microGeo#GeoPanelVarChangedProc,title=" "
		SetVariable m1y,help={"y direction of detector translator 1 (unit vector)"},format="%.5f"
		SetVariable m1y,limits={-1,1,0}, bodyWidth=50
		SetVariable m1z,pos={149,76+v},size={50,15},proc=microGeo#GeoPanelVarChangedProc,title=" "
	...
	if (mUseShow>=2)
		...
		SetVariable m2x,limits={-1,1,0}, bodyWidth=50
		SetVariable m2y,pos={85,76+v},size={55,15},proc=microGeo#GeoPanelVarChangedProc,title=" "
		SetVariable m2y,help={"y direction of detector translator 2 (unit vector)"},format="%.5f"
		SetVariable m2y,limits={-1,1,0}, bodyWidth=50
		SetVariable m2z,pos={149,76+v},size={50,15},proc=microGeo#GeoPanelVarChangedProc,title=" "
	...
	if (mUseShow>=3)
		...
		SetVariable m3x,limits={-1,1,0}, bodyWidth=50
		SetVariable m3y,pos={85,76+v},size={55,15},proc=microGeo#GeoPanelVarChangedProc,title=" "
		SetVariable m3y,help={"y direction of detector translator 3 (unit vector)"},format="%.5f"
		SetVariable m3y,limits={-1,1,0}, bodyWidth=50
		SetVariable m3z,pos={149,76+v},size={50,15},proc=microGeo#GeoPanelVarChangedProc,title=" "
		SetVariable m3z,help={"z direction of detector translator 3 (unit vector)"},format="%.5f"
		SetVariable m3z,limits={-1,1,0}, bodyWidth=50
	endif
	...
	Button buttonSaveLocal,pos={35,v+27},size={150,20},proc=microGeo#GeometryPanelButtonProc,title="Save in Current Fldr"
	Button buttonSaveLocal,help={"saves these values as default in the current data folder"}
	Button buttonEPICS,pos={145,v+49},size={40,20},proc=microGeo#GeometryPanelButtonProc,title="EPICS"
	Button buttonEPICS,help={"Fill the values in this panel from EPICS"}
	Button buttonFillFromWeb,pos={35,v+49},size={105,20},proc=microGeo#GeometryPanelButtonProc,title="Load from web"
	Button buttonFillFromWeb,help={"Fill the values in this panel from the web"}
	Button buttonFillFromFile,pos={35,v+71},size={150,20},proc=microGeo#GeometryPanelButtonProc,title="Load from a file"
	...
	if (exists("EPICS_put_PV_num")==6 && exists("EPICS_put_PV_str")==6)
		PopupMenu SaveGeoPopup,pos={35,v+95},size={150,20},bodyWidth=150,proc=microGeo#GeoSaveFilePopMenuProc
		...
		Button buttonWriteToFile,pos={35,v+93},size={150,20},proc=microGeo#GeometryPanelButtonProc,title="Write to a file"
		Button buttonWriteToFile,help={"Write values in this panel to a 'geoN' file"}
	endif
	Button buttonPrintGeo,pos={35,v+115},size={150,20},proc=microGeo#GeometryPanelButtonProc,title="Print Geo to History"
	Button buttonPrintGeo,help={"print geometry values to the history"}
	Button buttonCancel,pos={35,v+137},size={150,20},proc=microGeo#GeometryPanelButtonProc,title="Cancel"
...
Static Function UpDatePanelValuesFromStruct(win,iDetector)
	...
	if (m1Use)
		...
		SetVariable m1z, win=$win, value = _NUM:g.d[iDetector].m1[2]
	endif
	if (m2Use)
		SetVariable m2x, win=$win, value = _NUM:g.d[iDetector].m2[0]
		SetVariable m2y, win=$win, value = _NUM:g.d[iDetector].m2[1]
		SetVariable m2z, win=$win, value = _NUM:g.d[iDetector].m2[2]
	endif
	if (m3Use)
		SetVariable m3x, win=$win, value = _NUM:g.d[iDetector].m3[0]
		SetVariable m3y, win=$win, value = _NUM:g.d[iDetector].m3[1]
		SetVariable m3z, win=$win, value = _NUM:g.d[iDetector].m3[2]
	endif
...
Static Function GeoPanelUsedCheckBoxProc(cba) : CheckBoxControl
	...
	STRUCT microGeometry g								// geo that holds panels current values
	SVAR geoPanelStructStr = root:Packages:geometry:PanelValues:geoPanelStructStr
	SetGeoStructFromString(g,geoPanelStructStr)// fill temporary g, local to this function from geoPanelStructStr
...
Static Function GeoPanelVarChangedProc(sva) : SetVariableControl
	...
	elseif (StringMatch(sva.ctrlName,"m1x"))
		g.d[iDetector].m1[0] = sva.dval
	elseif (StringMatch(sva.ctrlName,"m1y"))
		g.d[iDetector].m1[1] = sva.dval
	elseif (StringMatch(sva.ctrlName,"m1z"))
		g.d[iDetector].m1[2] = sva.dval
	elseif (StringMatch(sva.ctrlName,"m2x"))
		g.d[iDetector].m2[0] = sva.dval
	elseif (StringMatch(sva.ctrlName,"m2y"))
		g.d[iDetector].m2[1] = sva.dval
	elseif (StringMatch(sva.ctrlName,"m2z"))
		g.d[iDetector].m2[2] = sva.dval
	elseif (StringMatch(sva.ctrlName,"m3x"))
		g.d[iDetector].m3[0] = sva.dval
	elseif (StringMatch(sva.ctrlName,"m3y"))
		g.d[iDetector].m3[1] = sva.dval
	elseif (StringMatch(sva.ctrlName,"m3z"))
		g.d[iDetector].m3[2] = sva.dval
...
Static Function GeoPanelDetectorDisable(win)			// Only enable/disable detector fields based on check box
	...
	ControlInfo/W=$win m1x
	if (abs(V_flag)==5)
		SetVariable m1x,win=$win, disable=disable;		SetVariable m1y,win=$win, disable=disable;		SetVariable m1z,win=$win, disable=disable
	endif
	ControlInfo/W=$win m2x
	if (abs(V_flag)==5)
		SetVariable m2x,win=$win, disable=disable;		SetVariable m2y,win=$win, disable=disable;		SetVariable m2z,win=$win, disable=disable
	endif
	ControlInfo/W=$win m3x
	if (abs(V_flag)==5)
		SetVariable m3x,win=$win, disable=disable;		SetVariable m3y,win=$win, disable=disable;		SetVariable m3z,win=$win, disable=disable
	endif
...
Static Function SetGeoStructFromPanelValues(g)	// set structure g to the values on the geo panel
	...
	SVAR geoPanelStructStr = root:Packages:geometry:PanelValues:geoPanelStructStr
	SetGeoStructFromString(g,geoPanelStructStr)	// set g with values from geoPanelStructStr ( the PanelValues)
	return 0
End
...
Function GeoFromEPICS(gIn)	// fill the geometry structure from EPICS (uses caget)
...
#ifdef EPICS_UPDATED
	geoList += "m1x1;m1y1;m1z1;m2x1;m2y1;m2z1;m3x1;m3y1;m3z1;"
	geoList += "m1x2;m1y2;m1z2;m2x2;m2y2;m2z2;m3x2;m3y2;m3z2;"
	geoList += "m1x3;m1y3;m1z3;m2x3;m2y3;m2z3;m3x3;m3y3;m3z3;"
#endif
	...
	for (i=0;i<MAX_Ndetectors;i+=1)
		...
		mvec[0] = NumberByKey("m1x"+si,pvList,"=")
		mvec[1] = NumberByKey("m1y"+si,pvList,"=")
		mvec[2] = NumberByKey("m1z"+si,pvList,"=")
		if (numtype(sum(mvec))==0 && norm(mvec)>0)
			g.d[i].m1[0] = mvec[0]
			g.d[i].m1[1] = mvec[1]
			g.d[i].m1[2] = mvec[2]
		...
		mvec[1] = NumberByKey("m2y"+si,pvList,"=")
		mvec[2] = NumberByKey("m2z"+si,pvList,"=")
		if (numtype(sum(mvec))==0 && norm(mvec)>0)
			g.d[i].m2[0] = mvec[0]
			g.d[i].m2[1] = mvec[1]
			g.d[i].m2[2] = mvec[2]
		...
		mvec[1] = NumberByKey("m3y"+si,pvList,"=")
		mvec[2] = NumberByKey("m3z"+si,pvList,"=")
		if (numtype(sum(mvec))==0 && norm(mvec)>0)
			g.d[i].m3[0] = mvec[0]
			g.d[i].m3[1] = mvec[1]
			g.d[i].m3[2] = mvec[2]
...
Static Function putDetector2EPICS(d,dnum)	// put detector values into EPICS, does not check d.used
	...
	mvec = d.m1[p]
	if (numtype(sum(mvec))==0 && norm(mvec)>0)
		if (geoLocal_putNum(EPICS_PREFIX+"m1x"+sd,mvec[0]))
			return 1
		endif
		if (geoLocal_putNum(EPICS_PREFIX+"m1y"+sd,mvec[1]))
			return 1
		endif
		if (geoLocal_putNum(EPICS_PREFIX+"m1z"+sd,mvec[2]))
			return 1
		endif
	endif
	mvec = d.m2[p]
	if (numtype(sum(mvec))==0 && norm(mvec)>0)
		if (geoLocal_putNum(EPICS_PREFIX+"m2x"+sd,mvec[0]))
			return 1
		endif
		if (geoLocal_putNum(EPICS_PREFIX+"m2y"+sd,mvec[1]))
			return 1
		endif
		if (geoLocal_putNum(EPICS_PREFIX+"m2z"+sd,mvec[2]))
			return 1
		endif
	endif
	mvec = d.m3[p]
	if (numtype(sum(mvec))==0 && norm(mvec)>0)
		if (geoLocal_putNum(EPICS_PREFIX+"m3x"+sd,mvec[0]))
			return 1
		endif
		if (geoLocal_putNum(EPICS_PREFIX+"m3y"+sd,mvec[1]))
			return 1
		endif
		if (geoLocal_putNum(EPICS_PREFIX+"m3z"+sd,mvec[2]))
			return 1
		endif
	endif
#endif
...
Static Structure microGeometryOLD		// structure definition
	int16	Ndetectors							// number of detectors in use, must be <= MAX_Ndetectors
	STRUCT detectorGeometryOLD d[6]		// geometry parameters for each detector
	STRUCT wireGeometry wire
	STRUCT sampleGeometry s
EndStructure
//
Static Structure detectorGeometryOLD	// structure definition for a detector
	int16 used							// TRUE=detector used, FALSE=detector un-used
	int32 Nx, Ny						// # of un-binned pixels in full detector
	double sizeX,sizeY				// outside size of detector (sizeX = Nx*pitchX), measured to outer edge of outer pixels (micron)
	double R[3]							// rotation vector (length is angle in radians)
...
EndStructure
//
// This allows reading old files with the microGeometryOLD structures
Static Function CopyOldGeoStruct2New(gNew,gOld)
	STRUCT microGeometryOLD &gOld
	STRUCT microGeometry &gNew

	gNew.Ndetectors = gOld.Ndetectors
	CopyWireGeometry(gNew.wire,gOld.wire)			// copy the wire
	CopySampleGeometry(gNew.s,gOld.s)				// copy Sample geometry

	Variable i
	for (i=0;i<(gOld.Ndetectors);i+=1)
		gNew.d[i].used = gOld.d[i].used
		gNew.d[i].Nx = gOld.d[i].Nx;			gNew.d[i].Ny = gOld.d[i].Ny
		gNew.d[i].sizeX = gOld.d[i].sizeX;	gNew.d[i].sizeY = gOld.d[i].sizeY
		gNew.d[i].R[0]=gOld.d[i].R[0];		gNew.d[i].R[1]=gOld.d[i].R[1];			gNew.d[i].R[2]=gOld.d[i].R[2];
		gNew.d[i].P[0]=gOld.d[i].P[0];		gNew.d[i].P[1]=gOld.d[i].P[1];			gNew.d[i].P[2]=gOld.d[i].P[2];
		gNew.d[i].timeMeasured = gOld.d[i].timeMeasured
		gNew.d[i].geoNote = gOld.d[i].geoNote
		gNew.d[i].detectorID = gOld.d[i].detectorID
		gNew.d[i].distortionMapFile = gOld.d[i].distortionMapFile
		gNew.d[i].rho00=gOld.d[i].rho00;	gNew.d[i].rho01=gOld.d[i].rho01;		gNew.d[i].rho02=gOld.d[i].rho02
...
End
//
Static Function SetGeoStructFromString(g,strStruct)	// set g to the values in strStruct
	STRUCT microGeometry &g						// connect to the globals
	String strStruct

	if (strlen(strStruct)==3600)					// an old strStruct
		STRUCT microGeometryOLD gOld
		StructGet/S gOld, strStruct				// fill gOld with the values stored in strStruct
		CopyOldGeoStruct2New(g,gOld)				// set g from values in gOld
	else
		StructGet/S g, strStruct					// fill g with the values stored in strStruct
	endif
	return 0
End
//
