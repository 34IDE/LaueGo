...
//					dxy = peakcorrection2(xymap,px,py)	// returns cmplx(dx,dy)
	...
	Wave Qvecs1keV = MakeQarray(image,geo,recip,Elo,Ehi,mask=mask,depth=depth)	// make an array the same size as an image, but filled with Q's for this depth, |Q| = 4*π*sin(theta) * E/hc
	KillWaves/Z image
