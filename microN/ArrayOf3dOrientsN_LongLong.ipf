...
#pragma version=2.51	// removed all of the old stuff, for 1.6 added the tensor parts, 2.0 changed surfer->gizmo
...
//		dezz_dZ =(Interp3d(ezz, xx,hh,ff+dZ) - ezz0)/dZ
	...
	MatrixOP/FREE temp = sqrt(sum(alpha*alpha))	// magnitude of tensor
...
Function curvatureTensorFromRods3d(xx,hh,ff,R3dX,R3dH,R3dF,kappa)
	...
	Make/N=(3,3)/D/FREE temp = kappa
	temp[0][0] -= Tr	;	temp[1][1] -= Tr	;	temp[2][2] -= Tr
	MatrixOP/FREE/O temp = sqrt(sum(temp*temp))	// magnitude of tensor
	Variable GND = temp[0] * GND_Density
...
Function SurfacePlotStyle(gName)
	...
	if (WhichListItem(SliceAngleAxis,"a;b;c;alpha;beta;gamma")>=0)	// display full auto-scale
		...
		ColorScale/C/N=colorScale1 image=$NameOfWave(image), heightPct=60, lowTrip=0.001,lblMargin=0,  tickUnit=1,ZisZ=1
	...
	elseif (!rgbWave)											// not an RGB wave
		...
		Variable colRange = isSymmetricScale(gName,image)
		if (numtype(colRange)!=0)								// it is symmetric, leave it alone
			WaveStats/Q/M=1 image				// these 3 lines make red negative, blue positive
			colRange = max(abs(V_min),abs(V_max))
	...
	elseif (WaveInClass(image,"OrientationSliceWave*"))
		...
		ColorScale/C/N=colorScale1 heightPct=60, lblMargin=0, lowTrip=0.001, tickUnit=1,ZisZ=1
...
Static Function sliceWave2RGB(sliceWave,SliceWaveIntens)
	...
	if (numtype(rmax))
//		Variable cutOff = WaveMax(SliceWaveIntens)/20
//		Duplicate/FREE sliceWave, sliceWaveTest
//		sliceWaveTest = SliceWaveIntens[p][q]<cutOff ? 0 : sliceWaveTest[p][q]
//		rmax = PickSymmetricRange(sliceWaveTest,trim)
...
Static Function Interpolate3dRodriquesFromXHF(resolution,[Xoff,Xwidth,Hoff,Hwidth,Foff,Fwidth])
	...
	if (strsearch(GetRTStackInfo(3),"LongLong,",0)!=0)
