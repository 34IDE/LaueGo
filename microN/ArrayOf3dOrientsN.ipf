...
#pragma version=2.72	// removed all of the old stuff, for 1.6 added the tensor parts, 2.0 changed surfer->gizmo
...
//
// Apr 16, 2009,  version 2.42
//	added scaling to XX, HH, & FF
//
// Aug 10, 2009,  version 2.43
//	changed InterpolateRodriquesFromXHF() & Interpolate3dRodriquesFromXHF(), to accept optional Xoff,Xwidth,Hoff,Hwidth,Foff,Fwidth
//	also Interpolate3dRodriquesClosest()
//
// Oct 22, 2009,  version 2.44
//	changed scaling of 3D-surface so that it uses sliceSquare properly. It scaled by sliceWaveAbs, but this gave bad colors for GND
//	changed Set3dSurfaceZscale() and  SetSliceProc()
//
// Oct 28, 2009,  version 2.45
//	changed Set3dSurfaceZscale() and  SetSliceProc() again to get the z-scaling correct
//
// Nov 17, 2009,  version 2.46
//	changed SetSliceProc(), forgot to delete sliceSquare wave
//	changed interpolate3dRodFromXHFSheets(), added possibility of intensity in waves sumAboveThreshold and totalSum
//	changed sliceWave2RGB() to take into account the intensity when selecting the range.  Ignore any that are < 5% of the max.
//
// Nov 21, 2009,  version 2.47
//	changed SurfacePlotStyle(), does not reset color scale position each time, also do a better job of deciding when to re-scale.
//
// Apr 27, 2010,  version 2.48
//	changed definition of GND from abs() to sqrt(sum of squares) in dislocationTensorFromRods3d() and in curvatureTensorFromRods3d().
//
// Apr 27, 2010,  version 2.49
//	changed definition of GND from abs() to sqrt(sum of squares) in dislocationTensorFromRods3d() and in curvatureTensorFromRods3d().
//
// May 21, 2010,  version 2.50
//	modified MakePanelSlicer(), Set3dSurfaceZscale(), and added SetZscale3dProc().  These three changes are all only for the 3D surface
//	of the SliceWave.  They put the fixed z-scale of the 3D surface on the PanelSlicer for easier use.
//
// June 11, 2010,  version 2.51
//	Changed MakeGraphOfSlice(), SurfacePlotStyle(), made the color scale transpearent and smaller
//
// Dec 17, 2010,  version 2.52
//  Changed coloring of slices so that they deal with unipolar values nicer, also use different ctabs for uni-polar and bi-polar
//	Changed SurfacePlotStyle(), SurfacePlotStyle(), made the color scale transpearent and smaller
//	Replaced PickSymmetricRange() and isSymmetricScale()   with   isUserSetScale(), PickUniOrSymmetricRange, and getColorScaleRange()
//	Added use of uniPolarCtab and biPolarCtab for specifying color tables
//
// Jan 13, 2011,  version 2.53
//  Fixed bad call to PickUniOrSymmetricRange() in SurfacePlotStyle()
//	Fixed bad call to SurfacePlotStyle() in SetSliceProc()
//	made curvatureTensorFromRods3d() a bit more compact
//
// Jan 13, 2011,  version 2.54
//	modified dislocationTensorFromRods3d() to return alpha, (alpha is no longer an argument)
//  modified curvatureTensorFromRods3d() to return kappa, (kappa is no longer an argument), Also made this ThreadSafe
//	added GNDfromCurvature() to make it easy to get GND from kappa, ThreadSafe too
//	modified SlicePlaneIn3d() to work with new dislocationTensorFromRods3d() and curvatureTensorFromRods3d()
//	added GetGNDdensity(), as a helpful utility
//
// Feb 4, 2011,  version 2.55
//	changed Change_GND_DislocationDensity() so that it get GND_density from xtal, easier and better. 
//		Can calculate directly from Lattice information.  I still do not understand where the GND_DislocationDensity_Al comes from.
//
// Sept 14, 2011,  version 2.56
//	changed SurfacePlotStyle() to make the file extension generic
//
// July 1, 2013,  version 2.57
//	made sure that all methods can use totalSum and sumAboveThreshold, also changed InterpolateRodriquesFromXHF() to ask the method to use.
//
...
//	// I was told (1¡/µm) = 1.36e10 (dislocations/cm^2) which should be 1.36e10*180/¹ = 77.92e10 for (rad/µm)
//	// for Al with a=4.05, I would guess that GND_DislocationDensity_Al=1e4/4.05e-8 = 24.691e10,  so where does 77.92e10 come from?
...
//		"( fix z scale",Set3dSurfaceZscale()
	...
	Submenu "   Read & Interpolate"
		"2nd Ñ Interpolate 3d grid",InterpolateRodriquesFromXHF(NaN)
		"1st Ñ Read In Raw Data",readInXYZorientationsFromFile(NaN)
	End
	"-"
...
Function/S BeamDirectionMenuItem(m)			// note, "!\022(" causes a menu item to be checked and disabled
	...
	if (m==0)
		return SelectString(flag,"!\022(","")+"beam from Right Ç--"
	else
		return SelectString(flag,"","!\022(")+"beam from Left   --È"
	endif
End
Function SetBeamDirection(flag)
	...
	if (strlen(WinList("PanelSlicer","","WIN:64")))					// update PanleSlicer if showing
		String str = SelectString(flag,"--È beam from Left","Ç-- beam from Right")
		Execute "PopupMenu popupBeamDirection win=PanelSlicer, value= "+"\""+str+"\""
		str = SelectString(flag,"Ç-- beam from Right","--È beam from Left")
...
Function/WAVE dislocationTensorFromRods3d(xx,hh,ff,exx,eyy,ezz,exy,exz,eyz,kappa)
	...
	Make/N=(3,3)/FREE/D alpha		// Dislocation Density Tensor
...
ThreadSafe Static Function/WAVE curvatureTensorFromRods3d(xx,hh,ff,R3dX,R3dH,R3dF)
	Variable xx,hh,ff					// desired xyz position
...
End
//
ThreadSafe Function GNDfromCurvature(xx,hh,ff,R3dX,R3dH,R3dF)	// this is sort of a utility routine to make it easier to use curvatureTensorFromRods3d() in some situations
	Variable xx,hh,ff					// desired xyz position
	Wave R3dX,R3dH,R3dF				// three components of the Roriques vectors in 3-d
	Wave kappa = curvatureTensorFromRods3d(xx,hh,ff,R3dX,R3dH,R3dF)
	return NumberByKey("GND", note(kappa),"=")
End
//
Function GraphTensorcomponents(type,[toPrint])
	...
	for (i=0,zrange=-Inf;i<9;i+=1)
		...
		Wave ww = $wName
		if (WaveExists(ww))
			zrange = max(zrange, imag(PickUniOrSymmetricRange(ww,0.025)))
//			zrange = max(zrange, ArrayOf3dOrients#PickSymmetricRange(ww,0.025))
	...
	for (i=0;i<9;i+=1)
		...
		RenameWindow #,$ab
		AppendImage $wName
		ModifyImage $wName ctab= {-zrange,zrange,$biPolarCtab,0}
...
//	so GND_Density = 1e12/b[] = dislocations/cm^2 = 1e11/b[nm] = dislocations/cm^2 = 1e15/b[nm] = dislocations/m^2
Function Change_GND_DislocationDensity(GND_Density)	// used to set the GND dislocation denstiy conversion factor
	...
	if (!(GND_Density>0) || ask)					// invalid input given
		...
		Prompt burgers,"Burgers Vector or Density", popup, "GND Density (1/cm^2);Burgers Vector (nm)"
		DoPrompt "GND conversion factor",burgers,GND_Density
		if (V_flag)
			return NaN
		endif
...
//	ColorScale/N=colorScale1/A=RC/X=-4.75/Y=0 image=sliceWave, heightPct=60
	ColorScale/N=colorScale1/A=RC/X=-4.75/Y=0/B=1 image=sliceWave, fsize=12, heightPct=40
...
Function Set3dSurfaceZscale(zmax)
	Variable zmax										// max z value (Inf->autoscale, NaN->ask user)
	...
	if (numtype(zmax)==2)
		...
		if (numtype(zmaxOld)==2)
			WaveStats/M=1/Q SliceWaveAbs
			zmax = roundSignificant(V_max,3)
		else
			zmax = zmaxOld
		endif
		Prompt zmax,"max Z value (use Inf to auto-scale)"
	...
	Wave sliceSquare=$SquaredVersionOfSurf(sliceWave)	// squared version of sliceWave
...
//		sliceSquare /= (4*zmax)
	...
	if (!autoScale)										// gives auto-scaling
		Note/K SliceWaveAbs, ReplaceNumberByKey("zmaxGizmo",note(SliceWaveAbs),zmax,"=")
	endif

	// update PanelSlicer if it is showing
	if (ItemsInList(WinList("PanelSlicer",";","WIN:64"))==1)
		SetVariable Zvalue3D win=PanelSlicer, value= _NUM:zmax
	endif
	return zmax
End
//
...
Function MakePanelSlicer(sliceW) : Panel	// panel to go with GraphSlice
	...
	SetVariable Zvalue3D,pos={112,84},size={100,19},title="z",proc=SetZscale3dProc
	SetVariable Zvalue3D,help={"scaled Z value for 3D plots"},fSize=12
	SetVariable Zvalue3D,limits={-inf,inf,0},value= _NUM:nan
...
Function SetZscale3dProc(sva) : SetVariableControl
	STRUCT WMSetVariableAction &sva
	if (sva.eventCode == 2)
		Set3dSurfaceZscale(numtype(sva.dval) ? Inf : sva.dval)
	endif
	return 0
End
...
Function SetSliceProc(ctrlName,value,varStr,varName) : SetVariableControl
	...
	Variable zmaxOld = Inf
	if (exists("sliceWaveAbs")==1)
		Wave sliceWaveAbs=sliceWaveAbs
		// force to auto-scale when changing axis, other wise try to use the previous axis type
		if (stringmatch(StringByKey("SliceAngleAxis",note(sliceWaveAbs),"="),StringByKey("SliceAngleAxis",noteStr,"=")))
			zmaxOld = NumberByKey("zmaxGizmo",note(sliceWaveAbs),"=")
		endif			
	endif

	Wave sliceSquare=$SquaredVersionOfSurf(sliceW)				// squared version of sliceWave
	...
	Variable zmax = numtype(zmaxOld)==0 ? zmaxOld : Inf			// prefer to use previous axis scale, otherwise auto-scale
	Set3dSurfaceZscale(zmax)
	KillWaves/Z sliceSquare
...
Function SurfacePlotStyle(gName)
	...
	if (WhichListItem(SliceAngleAxis,"a;b;c;alpha;beta;gamma")>=0)	// display full auto-scale
		...
		ColorScale/C/N=colorScale1 image=$NameOfWave(image), heightPct=40, lowTrip=0.001,lblMargin=0,  tickUnit=1,ZisZ=1
	...
	elseif (!rgbWave)											// not an RGB wave
		...
		if (!isUserSetScale(gName,image))					// if color range was not set by user, then...
			Variable/C colRange = PickUniOrSymmetricRange(image,0.015)
			if (numtype(real(colRange)+imag(colRange)))
				colRange = cmplx(-1,1) * -sqrt(2)*1e-4
			endif
			String ctabName = SelectString(real(colRange)*imag(colRange) == 0,uniPolarCtab,biPolarCtab)
			ModifyImage $NameOfWave(image) ctab= {real(colRange),imag(colRange),$ctabName,0}	// red is negative, white=0, blue is postive
		endif
		ColorScale/C/N=colorScale1 image=$NameOfWave(image), heightPct=40, lowTrip=0.001,lblMargin=0,  tickUnit=1,ZisZ=1
		ColorScale/W=$gName/C/N=colorScale1 StringByKey("DUNITS",WaveInfo(image,0))
	elseif (WaveInClass(image,"OrientationSliceWave*"))
		Variable rmin=NumberByKey("rmin",noteStr,"="), rmax=NumberByKey("rmax",noteStr,"=")
		ColorScale/C/N=colorScale1 heightPct=40, lblMargin=0, lowTrip=0.001, tickUnit=1,ZisZ=1
		ColorScale/C/N=colorScale1 ctab={rmin,rmax,$biPolarCtab,0}
		ColorScale/W=$gName/C/N=colorScale1 StringByKey("DUNITS",WaveInfo(image,0))
	endif
	if (newColorScale)
		ColorScale/C/N=colorScale1/A=RC/B=1/X=0/Y=0									// put new color scales in default position, otherwise, leave alone
	endif
...
//	SVAR imageExtension=root:Packages:imageDisplay:imageExtension
	String imageExtension= StrVarOrDefault("root:Packages:imageDisplay:imageExtension","")
	if (strlen(imageExtension)<1)
		initImageDisplayScaling()
		imageExtension =  StrVarOrDefault("root:Packages:imageDisplay:imageExtension","")
	endif
...
Static Function sliceWave2RGB(sliceWave,SliceWaveIntens)
	...
	Variable/C zrange
	if (!isUserSetScale(gName,sliceWave))
		zrange = PickUniOrSymmetricRange(sliceWave,trim)
	else
		zrange = getColorScaleRange(gName,sliceWave)
	endif
	rmin = real(zrange)
	rmax = imag(zrange)
	rmin = -rmax
//	rmax = isSymmetricScale(gName,sliceWave)
//	if (numtype(rmax))
...
//	Variable imax = PickSymmetricRange(SliceWaveIntens,trim)				// maximum intensity to use
	zrange = PickUniOrSymmetricRange(SliceWaveIntens,trim)				// maximum intensity to use
	Variable imax = imag(zrange)
//	print "imax = ",imax
...
Function InterpolateRodriquesFromXHF(resolution,[method,Xoff,Xwidth,Hoff,Hwidth,Foff,Fwidth])			// choose between the 2d and 3d interpolation
	Variable resolution	// resolution in final array (µm)
	String method
	Variable Xoff, Xwidth, Hoff, Hwidth, Foff, Fwidth
	Xoff = ParamIsDefault(Xoff) ? NaN : Xoff
	Xwidth = ParamIsDefault(Xwidth) ? NaN : Xwidth
	Hoff = ParamIsDefault(Hoff) ? NaN : Hoff
	Hwidth = ParamIsDefault(Hwidth) ? NaN : Hwidth
	Foff = ParamIsDefault(Foff) ? NaN : Foff
	Fwidth = ParamIsDefault(Fwidth) ? NaN : Fwidth
	...
	method = SelectString(ParamIsDefault(method),method,"")
	...
	if (strlen(method)<1)
		...
		elseif (dimX<=4 )
			method = "sheets"
		elseif (dimX>4)
			Variable imethod = 1						// defaults to full 3D
			Prompt imethod, "3D interpolation",popup,"Full 3D Interpolation;Choose Closest Point;Series of 2D Sheets"
			DoPrompt "3D Interpolation",imethod
			if (V_flag)
				return 1
			endif
			method = StringFromList(imethod-1,"3d;closest;sheets")
		endif
	endif
	...
	elseif (stringmatch(method,"closest"))
		...
		if (ParamIsDefault(Xoff) && ParamIsDefault(Xwidth) && ParamIsDefault(Hoff) && ParamIsDefault(Hwidth) && ParamIsDefault(Foff) && ParamIsDefault(Fwidth))
			Interpolate3dRodriquesClosest(resolution)
		else
			Interpolate3dRodriquesClosest(resolution,Xoff=Xoff,Xwidth=Xwidth,Hoff=Hoff,Hwidth=Hwidth,Foff=Foff,Fwidth=Fwidth)
		endif
	...
	elseif (stringmatch(method,"3d"))
		...
		if (ParamIsDefault(Xoff) && ParamIsDefault(Xwidth) && ParamIsDefault(Hoff) && ParamIsDefault(Hwidth) && ParamIsDefault(Foff) && ParamIsDefault(Fwidth))
			...
			Interpolate3dRodriquesFromXHF(resolution,Xoff=Xoff,Xwidth=Xwidth,Hoff=Hoff,Hwidth=Hwidth,Foff=Foff,Fwidth=Fwidth)
		endif
	else
...
Static Function Interpolate3dRodriquesFromXHF(resolution,[Xoff,Xwidth,Hoff,Hwidth,Foff,Fwidth])
	Variable resolution	// resolution in final array (µm)
	Variable Xoff, Xwidth, Hoff, Hwidth, Foff, Fwidth
	Xoff = ParamIsDefault(Xoff) ? NaN : Xoff
	Xwidth = ParamIsDefault(Xwidth) ? NaN : Xwidth
	...
	Variable N=numpnts(RX)
	Wave totalPeakIntensity=totalPeakIntensity, totalIntensity=totalIntensity		// these may not exist any more
	if (!WaveExists(totalPeakIntensity))											// old style name does not exist, try new one
		Wave totalPeakIntensity=sumAboveThreshold
	endif
	if (!WaveExists(totalIntensity))												// old style name does not exist, try new one
		Wave totalIntensity=totalSum
	endif
	...
	if (numtype(Xwidth+Xoff)==0)
		Xwidth = round(Xwidth/resolution)*resolution
		Xlo = resolution*floor(Xoff/resolution)			// low end of 3d volume
		NX = round(Xwidth/resolution)+1
	else
		FindScalingFromVec(XX,threshold,Xoff,dX,dimX)
		Xlo = resolution*floor(Xoff/resolution)			// low end of 3d volume
	...
	if (numtype(Hwidth+Hoff)==0)
		Hwidth = round(Hwidth/resolution)*resolution
		Hlo = resolution*floor(Hoff/resolution)
		NH = round(Hwidth/resolution)+1
	else
	...
	if (numtype(Fwidth+Foff)==0)
		Fwidth = round(Fwidth/resolution)*resolution
		Flo = resolution*floor(Foff/resolution)
		NF = round(Fwidth/resolution)+1
	else
		FindScalingFromVec(FF,threshold,Foff,dF,dimF)
		Flo = resolution*floor(Foff/resolution)
		NF=ceil((dimF*dF+Foff-Flo)/resolution)
	endif
	NF = max(NF,1)									// have to have at least one point (no intervals)
	...
	if (WaveExists(totalPeakIntensity))
		...
		print "done with totalPeakIntensity"
	endif
	if (WaveExists(totalIntensity))
		...
		print "done with totalIntensity"
	endif
...
End
//
Static Function Interpolate3dRodriquesClosest(resolution,[Xoff,Xwidth,Hoff,Hwidth,Foff,Fwidth])
	Variable resolution	// resolution in final array (µm)
	Variable Xoff, Xwidth, Hoff, Hwidth, Foff, Fwidth
	Xoff = ParamIsDefault(Xoff) ? NaN : Xoff
	Xwidth = ParamIsDefault(Xwidth) ? NaN : Xwidth
	...
	Wave XX,HH,FF		// positions (beamline coords) for the (RX,RH,RF)
	Wave RX,RH,RF		// three components of the Rodriques vector
	Wave totalIntensity,totalPeakIntensity											// these may not exist any more
	if (!WaveExists(totalPeakIntensity))											// old style name does not exist, try new one
		Wave totalPeakIntensity=sumAboveThreshold
	endif
	if (!WaveExists(totalIntensity))												// old style name does not exist, try new one
...
Static Function interpolate3dRodFromXHFSheets(resolution,[clipWave,clipValueMin])
	...
	FindScalingFromVec(HH,threshold,Hoff,dH,dimH)
	FindScalingFromVec(FF,threshold,Foff,dF,dimF)
	printf "from raw data in range:  [%g,%g] [%g,%g] [%g,%g], N = (%d, %d, %d),  Æ=(%g, %g, %g)\r",Xoff,Xoff+dX*(dimX-1),Hoff,Hoff+dH*(dimH-1),Foff,Foff+dF*(dimF-1),dimX,dimH,dimF,dX,dH,dF
	//	printf "X=[%g,%g], dimX=%d,  ÆX%.3f\r",Xoff,Xoff+dX*(dimX-1),dimX,dX
	//	printf "H=[%g,%g], dimH=%d,  ÆH=%.3f\r",Hoff,Hoff+dH*(dimH-1),dimH,dH
	//	printf "F=[%g,%g], dimF=%d,  ÆF=%.3f\r",Foff,Foff+dF*(dimF-1),dimF,dF
	Variable Xlo, Hlo, Flo									// low end of 3d volume
	Variable Xhi, Hhi, Fhi									// high end of 3d volume
	Variable NX, NH, NF									// number of points (not intervals) along each axis
	//	Xlo = resolution*floor(Xoff/resolution)			// low end of 3d volume
...
Static Function isUserSetScale(gName,image)		// was the color range for the z-scale set by the user?
	String gName
	Wave image

	Variable/C zrange = getColorScaleRange(gName,image)
	Variable lo=real(zrange), hi=imag(zrange)
	Variable symmetric = (lo+hi)==0
	Variable zeroBased = (lo==0) || (hi==0)
	Variable bothZero = (lo==0) && (hi==0)
	if ((symmetric|| zeroBased) && placesOfPrecision(hi)<5 && !bothZero)
		return 1									// range was probably set by user
	endif
	return 0										// range was probably auto set to 'odd' values
End


Static Function/C PickUniOrSymmetricRange(waveIn,frac)	// provides and appropriate color range for this wave
	Wave waveIn
	Variable frac
	Duplicate/O/FREE waveIn, w
	Redimension/N=(numpnts(waveIn)) w		// changes multi-dimensional waves to 1-D
	SetScale/P x 0,1,"", w
	Sort w w
	WaveStats/M=1/Q w

	Variable n = V_npnts-1
	Variable xlo=w[n*frac], xhi=w[n*(1-frac)]
	Variable maxVal = max(abs(xlo),abs(xhi))	// maximal outside value
	if (V_max*V_min > 0)						// wave has + and - values
		xlo = -maxVal
		xhi = maxVal
	elseif (V_min<0)							// wave has only - values
		xlo = -maxVal
		xhi = 0
	elseif (V_max<0)							// wave has only + values
		xlo = 0
		xhi = maxVal
	else											// really bad values
		xlo = NaN
		xhi = NaN
	endif
...
Static Function/C getColorScaleRange(gName,image)		// get color range in use on the graph
	String gName
	Wave image
	String str = StringByKey("RECREATION",ImageInfo(gName,NameOfWave(image),0))	//ctab= {*,*,RedWhiteBlue,0}
	str = StringByKey("ctab",str,"=")
	str = str[2,strlen(str)-2]
	Variable lo = str2num(StringFromList(0,str,","))
	Variable hi = str2num(StringFromList(1,str,","))
	return cmplx(lo,hi)
End


//   The funciton    PickSymmetricRange()  is NO LONGER USED
Static Function PickSymmetricRange(wIn,frac)
	Wave wIn
	Variable frac
	Duplicate/FREE wIn, w
	Redimension/N=(numpnts(wIn)) w
	SetScale/P x 0,1,"", w
	Sort w w
	WaveStats/M=1/Q w
	Variable n = V_npnts-1
...
//   The funciton    isSymmetricScale()  is NO LONGER USED
// for a graph check if image has a symmetric range, if so return value, otherwise return NaN
Static Function isSymmetricScale(gName,image)
	String gName
	Wave image
	...
	Variable lo = str2num(StringFromList(0,str,","))
	Variable hi = str2num(StringFromList(1,str,","))
	if ((lo+hi)==0 && placesOfPrecision(hi)<5)
		return hi
	endif
	return NaN
End
