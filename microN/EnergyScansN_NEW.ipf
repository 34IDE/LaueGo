...
//					dxy = peakcorrection2(xymap,px,py)			// returns cmplx(dx,dy)
	...
	Wave Qvecs1keV = MakeQarray(image,geo,recip,Elo,Ehi,mask=mask,depth=depth,printIt=printIt)	// Qvecs1keV[Ni][Nj][3]
	if (mono)
...
Static Function/S Fill3DQhist1image(image,Qvecs1keV,Qhist,QhistNorm,[mask,dark,I0normalize,printIt])// fill Qhist from one file, F is for fast, sin(theta) is precomputed
	Wave image
	Wave Qvecs1keV										// pre-computed array of sin(theta)'s that is same size as the image to be loaded, [Ni][Nj][3]
...
//	MatrixOP/FREE Qvecs = Qvecs1keV * keV			// actual Q-vector for each pixel
	...
	MatrixOP/FREE Qijx = round((Qvecs1keV[][][0]*keV - Qx0) / dQx)		// a [Ni][Nj] array
	MatrixOP/FREE Qijy = round((Qvecs1keV[][][1]*keV - Qx0) / dQx)
	MatrixOP/FREE Qijz = round((Qvecs1keV[][][2]*keV - Qx0) / dQx)
...
//				QhistNorm[i][j][k] += 1
//				Qhist[Qijk[0][ip][jp]][Qijk[1][ip][jp]][Qijk[2][ip][jp]]		 += image[ip][jp] * inorm
//				QhistNorm[Qijk[0][ip][jp]][Qijk[1][ip][jp]][Qijk[2][ip][jp]] += 1
...
// Make an array the same size as an image, but filled with Q^ to each pixel's
// for a 2K x 2K image, this takes 1 minute.
// changed so dimensions: [3][Nx][Ny] --> [Nx][Ny][3]
Static Function/WAVE MakeQarray(image,geo,recip,Elo,Ehi,[depth,mask,printIt])
	Wave image
	STRUCT microGeometry &geo						// the input geo
	Wave recip											// reciprocal lattice, used to find range of Q's
...
//	Make/N=(3,Ni,Nj)/D/FREE Qvecs1keV=NaN				// Qvector at 1keV
	...
	Wave qvecs = pixel2qVEC(geo.d[dNum],pxpy,depth=depth)	// qvecs in beam line coords, at this point qvecs are normalized, qvecs[N][3]
