...
Function Fill_Q_Positions(d0,pathName,nameFmt,range1,range2,mask,[depth,maskNorm,dark,I0normalize,printIt])	// does not assume depth in image
	...
	if (Nz<=1 && numtype(depth0))
		...
		DoPrompt "scan sizes",dkeV,NkeV,dx,Nx,dy,Ny,ddep,Nz
		ddep = (ddep==0) ? 1 : ddep
	endif
	if (V_flag)
		return 1
...
Function Fill1_3DQspace(recipSource,pathName,namePart,range,[depth,mask,dark,doConvex,printIt])	// does not assume depth in image
	...
	if (printIt)
		Variable deV = 1000*(keV_FillQvsPositions[ikeVhi]-keV_FillQvsPositions[ikeVlo])/(NkeV-1)
		printf "E range = [%g, %g] (keV),  ∆E=%.2g eV", keV_FillQvsPositions[ikeVlo], keV_FillQvsPositions[ikeVhi],deV
...
//					dxy = peakcorrection2(xymap,px,py)			// returns cmplx(dx,dy)
	...
	Wave Qvecs1keV = MakeQarray(image,geo,recip,Elo,Ehi,mask=mask,depth=depth,printIt=printIt)	// make an array the same size as an image, but filled with Q's at 1keV for this depth, |Q| = 4*π*sin(theta) * E/hc
	if (mono)
