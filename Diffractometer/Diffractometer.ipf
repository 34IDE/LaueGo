...
//		MatrixOP/FREE/O kf = rot x ki
	normalize(kf)								// change length of kf to 2π/lambda
	kf *= 2*PI/(ref0.lambda)
	Wave Tmat = diffractMatrix(A)
...
Static Function DetectorBad(d)
	...
	bad += (abs(d.R[0])>2*PI || abs(d.R[1])>2*PI || abs(d.R[2])>2*PI)		// rotation cannot be more than 2π
...
Static Function ResetSetPilatus100Kcalibration(xydir,px0,py0,dist)
	...
	printf "Using Nx=%g,  Ny=%g,  ∆pixel=%g µm\r",Nx,Ny,dpixel
	printf "R[3] = \"%s\"\r",Rstr
